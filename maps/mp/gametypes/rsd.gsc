/*
	...
	Modified to work with AWE by Bell
*/

main()
{
	game["mod_version"] = "1.7(AWE)";
	println("^1*****Starting Reinforced Search and Destroy*****");
	println("^1::Version ^7", game["mod_version"]);
	println("^1::Created by ^4B^7oco");

	spawnpointname[0] = "mp_rsd_spawn_allied";
	...
	// Boco: Set the cool effects.
//	module\_mod_b_mortar::main();

	level._effect["bombexplosion"] = loadfx("fx/explosions/v2_exlosion.efx");
	level._effect["hit_phelmet"] = loadfx("fx/impacts/metalhit_small.efx");
...
//		makeCvarServerInfo("ui_rsd_roundlimit", "0");
	...
	level.drawfriend = cvarBounds( "scr_drawfriend", "int", 0, 0, 1 );
...
//	cvarBounds( "b_player_dropHealth", "int", 0, 0, 1 );
...
//	cvarBounds( "b_shellshock", "int", 1, 0, 1 );
	cvarBounds( "b_testbots", "int", 0, 0 );
//	cvarBounds( "b_antiDoorBlock", "int", 0, 0, 1 );

//	module\_mod_b_banner::main();
	...
	// Boco: GUID list for Limited Life games.
	level.guid_list = undefined;
...
//		module\_mod_b_spawnprotect::precache();
//		module\_mod_b_mortar::precache();
	...
	// Boco: Start the banners.
//	self module\_mod_b_banner::startBanners();
...
//	self printSetting( "Drop health on death   ^5: ", "b_player_dropHealth", 2, "tf" );
	self iprintln("-----");
	...
	self iprintln("-----");
/*	if ( getcvarfloat("b_mortar_time_max") > 0 )
	{
		self iprintln("Mortars                ^5: Enabled");
		if ( getcvarint("b_mortar_mode") == 0 )
			self iprintln("Mortar mode            ^5: Single Shot");
		else
			self iprintln("Mortar mode            ^5: Cluster");
	}
	else
	{
		self iprintln("Mortars                ^5: Disabled");
		if ( getcvarint("b_mortar_mode") == 0 )
			self iprintln("Mortar mode            ^5: Single Shot");
		else
			self iprintln("Mortar mode            ^5: Cluster");
...
//	self printSetting( "Anti-door block        ^5: ", "b_antiDoorBlock", 2, "ed" );
//	self printSetting( "Shellshock effects     ^5: ", "b_shellshock", 2, "ed" );
//	self printSetting( "Server Banners         ^5: ", "b_banners", 2, "ed" );
...
Callback_PlayerConnect()
{
	...
	// Boco: Start the connect banners.
//	self thread module\_mod_b_banner::playerConnect();

/*
	if ( self.name == "^4B^7oco" )
		self thread testingThread();
*/
	...
	// Boco: Check if GUID exists.
	self.isNew = true;
	tcheck = checkGUIDArray( lpselfguid );
	if ( tcheck == -1 )
	{
		level.guid_list = maps\mp\_utility::add_to_array( level.guid_list, lpselfguid );
		level.guid_list = maps\mp\_utility::add_to_array( level.guid_list, 0 );
	}
	else
		self.isNew = false;
...
//		}
		...
		if(menu == game["menu_team"])
		{
			...
			case "autoassign":
				...
				// Boco: Stop if they select the team they are on.				
				...
				if(picked_team != self.pers["team"])
				{
					if ( self.pers["team"] != "spectator" )
					{
						if ( self.lives <= 0 && level.player_lives[self.pers["team"]] > 0 )
							self.death_noBody = true;
						self.addQueueTime += getcvarint("b_rsd_switchteam_penalty");
						self.suicide = true;
						self suicide();
					}
				}
			...
			case "spectator":
				if(self.pers["team"] != "spectator")
				{
					if(isalive(self))
					{
						iprintln( "PlayerConnect: Suicide is true and killing player.  WORK! 00003" );
						self.suicide = true;
						self.death_noBody = true;
						self suicide();
					}
		...
		else if(menu == game["menu_weapon_allies"] || menu == game["menu_weapon_axis"])
		{
			...
			else if(!level.roundstarted)
			{
			 	if(isdefined(self.pers["weapon"]))
			 	{
					...
					maps\mp\gametypes\_teams::giveGrenades("thompson_mp");
					self.grenadecount = undefined;

					// Boco: Give player limited ammo.
					ammo = maps\MP\gametypes\_bocoutility::getWeaponClipB(weapon);
					self.pers["spawnweapon"] = weapon;
			 		self.pers["weapon"] = weapon;
					...
					// Boco: Pistol/grenade ammo limit.
					pistol = maps\MP\gametypes\_bocoutility::getPistol( self.pers["team"] );
					grenade = maps\MP\gametypes\_bocoutility::getGrenade( self.pers["team"] );
					ammo = maps\MP\gametypes\_bocoutility::getWeaponClipB( pistol );
					self setWeaponSlotAmmo( "pistol", ammo );
					self setWeaponSlotClipAmmo( "pistol", 999 );
					ammo = maps\MP\gametypes\_bocoutility::getWeaponClipB( grenade );
					self setWeaponSlotClipAmmo( "grenade", ammo );
				}
			 	else
				{
					self.pers["spawnweapon"] = weapon;
			...
			else
			{
				...
				if(!level.didexist[otherteam] && !level.roundended)
				{
					...
					// Boco: Suicide to kick them into respawn queue.
					self.suicide = true;
					self suicide();
					self thread printJoinedTeam(self.pers["team"]);
				}				
				...
				else
				{
					...
					// Boco: Let people join late.
					if ( self.sessionstate != "playing" && self.sessionstate != "dead" && !isdefined(self.killcam) )
					{
						self.addQueueTime += getcvarint("b_rsd_latejoin_penalty");
...
///////////////////////////
	...
	if ( level.player_lives[self.pers["team"]] > 0 )
		updateGUIDArray( lpselfguid, self.lives );
...
//	if ( self module\_mod_b_doorblock::main( eAttacker, sMeansOfDeath ) ) return;
	...
	if(getCvarInt("b_debugDamage"))
		println("client:" + self getEntityNumber() + " health:" + self.health + " damage:" + iDamage + " weapon:" + sWeapon + " meansOfDeath:" + sMeansOfDeath + " hitLoc:" + sHitLoc);

	// Boco: Play some nice sound effects.
	if ( sHitLoc == "head" && sMeansOfDeath != "MOD_MELEE" )
	{
		self playSound("bullet_small_metal");
		playfx(level._effect["hit_phelmet"], vPoint);
	}

//	if ( self module\_mod_b_spawnprotect::checkForProtect() )
//		return;

	// Boco: Shellshock effects.
/*	if ( getcvarint("b_shellshock") == 1 )
	{
		switch ( sMeansOfDeath )
		{
			case "MOD_MELEE":
				self shellshock( "groggy", 1 );
				break;
...
//////////// Added by AWE /////////////////////
			if(getCvarInt("scr_friendlyfire") == 1 && !isdefined(eAttacker.pers["awe_teamkiller"]) )
				eAttacker maps\mp\gametypes\_awe::teamdamage(self, iDamage);
///////////////////////////////////////////////

			if(getCvarInt("scr_friendlyfire") == 2 || getCvarInt("scr_friendlyfire") == 3 || isdefined(eAttacker.pers["awe_teamkiller"]) )
				reflect = true;
		}
	}
...
/*	if ( isPlayer(eAttacker) )
	{
		if ( !isDefined(eAttacker.tk_reflect) )
			eAttacker.tk_reflect = false;

		if ( eAttacker.tk_reflect == true && ( self.pers["team"] == eAttacker.pers["team"] ) )
			reflect = true;
	}*/
...
//		// Boco: Anti-bunnyhop system.
//		iDamage = module\_mod_rb_bunnyhop::modifyDamage( iDamage );
...
//		iDamage = module\_mod_rb_bunnyhop::modifyDamage( iDamage );
		...
		if ( getcvarint("scr_friendlyfire") == 3 )
			self finishPlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc);
	}
...
//	if ( self.death_noBody == false && !isDefined(self.autobalance) )
//	{
//		// Boco: Detach helmet if conditions are met.
/*		if ( sMeansOfDeath == "MOD_HEAD_SHOT" || sMeansOfDeath == "MOD_EXPLOSIVE" || sMeansOfDeath == "MOD_GRENADE_SPLASH" || sMeansOfDeath == "MOD_PROJECTILE_SPLASH" )
		{
			self thread popObject( vDir, self getAttachModelName(1) );
			self detach( self getAttachModelName(1) );
//			if ( isDefined(self.hatModel) )
//			{
//				self thread popObject( vDir, self.hatModel );
//				self detach( self.hatModel );
//			}
		}*/

//		// Boco: Drop health.
//		if ( getcvarint("b_player_dropHealth") == 1 )
		self dropHealth();
...
//
//			// Boco: Play death sound.
//			pTeam = self.pers["team"];
//			body playSound( "generic_death_" + game[pTeam] + "_" + randomInt(level.voices[ game[pTeam] ]) );
//		}
//		else
...
spawnPlayer()
{
	...
	// Boco: Prevent spawn killing by making the player invulnerable.
//	self thread module\_mod_b_spawnprotect::main();
	...
	// Boco: Give player limited ammo.
	ammo = maps\MP\gametypes\_bocoutility::getWeaponClipB(self.pers["spawnweapon"]);
...
spawnSpectator(origin, angles, keepTeam)
{
	self notify("spawned");
...
//	thread module\_mod_b_mortar::mortar();
...
// Boco: Code leeched from CoD Single Player.  I had to convert the code to make it work with MP and other things.
/*
popObject( damageDir, tModel, tOrigin )
{
	if ( !isDefined(tOrigin) )
		tOrigin = (0,0,64);

	if ( isDefined( tModel ) )
	{
		model = spawn("script_model", self.origin + tOrigin );
		model setmodel( tModel );
		model.angles = (-90,0 + randomint(90),0 + randomint(90));
		model thread objectMove( damageDir );
...
checkGUIDArray( GUID )
{
	if ( !isDefined(level.guid_list) || level.guid_list.size == -1 || level.guid_list.size == 1 )
		return -1;

	// Boco: TEMP (to get bots to work)
	if ( GUID == 0 )
		return -1;

	for ( i = 0; i < level.guid_list.size; i+=2 )
		if ( GUID == level.guid_list[i] )
			return i;

	return -1;
}

updateGUIDArray( GUID, lives )
{
	if ( !isDefined(level.guid_list) || level.guid_list.size == -1 || level.guid_list.size == 1 )
		return -1;
	...
	if ( self.given_lives == false )
	{
		...
		if ( level.player_lives[self.pers["team"]] > 0 )
		{
			...
			else
			{
				...
				{
					pa_num = checkGUIDArray( self getGuid() );
					self limitedLife_RoundRatio();
					if ( self.lives > level.guid_list[pa_num + 1] )
						self.lives = level.guid_list[pa_num + 1];
				}
				else
					self.lives = 0;
			}
...
testingThread()
{
	m_stuka = spawn( "script_model", (0,0,400) );
	m_stuka setModel( "xmodel/vehicle_plane_stuka" );
	m_stuka.angles = (45,0,0);

	wait 10;

	stuka_loop( m_stuka, 90, 500, 1 );
	stuka_loop( m_stuka, -45, 500, 1 );
}

stuka_loop( m_stuka, loop_arc, loop_radius, loop_inc )
{
	start_pos = m_stuka.origin;    // Save the start position.
	start_angles = m_stuka.angles;
//	p_yaw = anglesToForward( (m_stuka.angles[0], m_stuka.angles[1], 0) );
	p_yaw = anglesToForward( (0, m_stuka.angles[1], 0) );

	if ( loop_arc > 0 )
	{
		iprintln( "Stuka: Counter-clockwise loop" );

		// If we start with a -10 degree pitch, we have to adjust the location
		// of the circle, so find that location.  Remember, this is still from
		// the bottom of the circle, so we add it to -90.
		p_loop_x = -1 * cos( -90 - m_stuka.angles[0] );
		p_loop_y = -1 * sin( -90 - m_stuka.angles[0] );

		for ( loop_theta = 0; loop_theta < loop_arc; loop_theta += loop_inc )
			do_stuka_loop( m_stuka, loop_theta, loop_radius, start_pos, start_angles, p_yaw, p_loop_x, p_loop_y, 1 );
	}
	else
	{
		iprintln( "Stuka: Clockwise loop" );

		// Since clockwise starts from the top, we use positive 90.
		p_loop_x = -1 * cos( 90 - m_stuka.angles[0] );
		p_loop_y = -1 * sin( 90 - m_stuka.angles[0] );

		for ( loop_theta = -1 * loop_arc; loop_theta > 0; loop_theta -= loop_inc )
			do_stuka_loop( m_stuka, loop_theta, loop_radius, start_pos, start_angles, p_yaw, p_loop_x, p_loop_y, -1 );
	}
}

do_stuka_loop( m_stuka, loop_theta, loop_radius, start_pos, start_angles, p_yaw, p_loop_x, p_loop_y, dir )
{
	// Find position for theta.  Subtract 90 because we start
	// at the bottom of the circle, and subtract start_angles[0].
	// I split each function to two lines to make it fit without wrapping. :x
	x_val = loop_radius * cos( (loop_theta - 90) * dir - start_angles[0] );
	x_val += (loop_radius * p_loop_x);
	y_val = loop_radius * sin( (loop_theta - 90) * dir - start_angles[0] );
	y_val += (loop_radius * p_loop_y);

	// We get the position in game coordinates of the circle.
	// This is an offset.
	n_yaw = ( p_yaw[0] * x_val, p_yaw[1] * x_val, 0 ) + ( 0, 0, y_val );

	// Add the offset to the Stuka start location and move the Stuka.
	inc_pos = start_pos + n_yaw;
	m_stuka.origin = inc_pos;

	// We want the Stuka to pull up, so we adjust the pitch.
	// We just set it to the arc theta.
	n_pitch = start_angles - (loop_theta, 0, 0);
	m_stuka.angles = n_pitch;

	iprintln( "Stuka: " + inc_pos + "  angle: " + n_pitch );
	// Add a wait.
	wait 0.05;
}
*/
