...
//////////////////////////////////
	...
	// Half-Time Team Switch - Added by Kraken (Paul)
	currentRoundNumber = game["roundsplayed"] + 1;
	sdRoundLimit = getCvarInt("scr_sd_roundlimit");

	if (isHalfTime(currentRoundNumber, sdRoundLimit))
	{
		doTeamSwap();
	}
...
///////// Changed by AWE //////////////
					if(isdefined(level.awe_showsdtimer))
					{
...
isHalfTime(currentRoundNumber, maxRoundNumber)
{
	// Should work for an even or odd maxRoundNumber, but odd is preferred as the switch costs one round.
	if (currentRoundNumber == (((int)(maxRoundNumber / 2)) + 1))
	{
		iprintlnbold("^2It's half-time!");
		wait 2;
		return true;
	}
...
doTeamSwap()
{
	// Taken from CoD1 PAM mod (small edits by Kraken)

	// Print message
	iprintlnbold("Switching teams");
	wait 3;
	...
	// Switch teams, swap models and clean up
	players = getentarray("player", "classname");
