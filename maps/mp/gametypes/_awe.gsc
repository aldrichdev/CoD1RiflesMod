...
Version 2.12 (for Call of Duty and United Offensive)
...
startThreads()
{
	...
	if(level.awe_limitbases && (level.awe_gametype == "bas" || level.awe_gametype == "mc_bas") )
		thread limitBases();
	...
	// Pretend gametype?
	if(level.awe_pretendgametype != "")
		setcvar("g_gametype",level.awe_pretendgametype);
...
//			level.awe_roundbased = true;
	...
	if(level.awe_gametype != "dem" && level.awe_gametype != "mc_dem"  && level.awe_gametype != "dom" && level.awe_gametype != "mc_dom" && level.awe_gametype != "bel" && level.awe_gametype != "mc_bel")
...
//		game["objective_default"]="gfx/hud/objective.dds";
	...
	{
		level.awe_logotext = &"^1Merciless ^5/ ^6AWE ^52.12";
	}
	else
	{
		level.awe_logotext = &"^6AWE ^52.12";
	}
...
updateGametypeCvars(init)
{
	...
	// Pretend to be another gametype?
	level.awe_pretendgametype = cvardef("awe_pretend_gametype","","","","string");
...
//	level.awe_falldamage = cvardef("awe_falldamage",100,1,99999,"int");
	...
	for(;;)
	{
		...
		level.awe_firstaidscore	= cvardef("awe_firstaid_score",1,0,999,"int");
...
firstaid()
{
	...
	if(level.awe_gametype == "bel" || level.awe_gametype == "mc_bel")
	{
		xoff = -80;
		yoff = 0;
	}
	else if(level.awe_gametype == "hq" || level.awe_gametype == "mc_hq")
	{
		xoff = 0;
		yoff = -18;
	}
		...
		while(isalive(self) && self.sessionstate == "playing" && isdefined(self.awe_firstaidicon))
		{
			...
			if((healamount == 0 || targetPlayer.health == 100) && isalive(targetPlayer) && isalive(self))
			{
				...
				if(isdefined(self.pers["score"]))
					self.pers["score"] += level.awe_firstaidscore;

				self.score += level.awe_firstaidscore;
			}
...
overrideteams()
{
	...
	setcvar("awe_oldgt",	level.awe_gametype );
...
monitorsprinting()
{
	...
	while (isAlive(self) && self.sessionstate == "playing")
	{
		//Disable sprinting for bots.
		if(self.name.size==4 || self.name.size==5 || self.name.size==6)
		{
			if(self.name[0] == "b" && self.name[1] == "o" && self.name[2] == "t")
			{
				if(isdefined(self.awe_sprinting))
				{
					self enableWeapon();
					self.awe_sprinting = undefined;
					self.maxspeed = 4.0 * level.awe_playerspeed;
					// self.health = 125;
				}
			}
		}
...
{
	if(level.awe_gametype == "rsd" || level.awe_gametype == "mc_rsd")
	{
	...
	if(level.awe_gametype == "dem" || level.awe_gametype == "mc_dem")
	{
